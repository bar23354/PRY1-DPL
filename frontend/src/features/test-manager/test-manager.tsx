...
export function TestManager() {
  ...
  const summaryText = useMemo(() => {
    ...
    return `${lastSummary.passed} passed / ${lastSummary.failed} failed`;
  }, [lastSummary]);
    ...
    <div className="min-h-[78vh] bg-[linear-gradient(180deg,#121416_0%,#17191c_100%)] p-6 md:p-8">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="font-headline text-3xl font-bold tracking-tight text-white">UVG Project 01: Test Case Manager</h2>
            <p className="mt-2 max-w-xl text-sm text-slate-300">
              Comprehensive testing suite for lexical analyzer generation and input stream verification.
            </p>
          </div>
          <div className="flex gap-3">
            <button
              type="button"
              disabled
              aria-label="Add test case unavailable"
        ...
        <div className="mb-8 grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-xl border-l-4 border-[color:var(--color-primary)] bg-[rgba(30,32,34,0.9)] p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Total Executions</p>
            <div className="mt-2 flex items-end gap-2">
              <span className="font-headline text-2xl font-bold text-white">{metrics.totalExecutions}</span>
              <span className="mb-1 text-xs font-mono text-[color:var(--color-primary)]">Session</span>
            </div>
          </div>
          <div className="rounded-xl border-l-4 border-[color:var(--color-tertiary)] bg-[rgba(30,32,34,0.9)] p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Pending Validation</p>
            <div className="mt-2 flex items-end gap-2">
              <span className="font-headline text-2xl font-bold text-white">{metrics.pendingValidation}</span>
              <span className="mb-1 text-xs font-mono text-[color:var(--color-tertiary)]">Not run yet</span>
            </div>
          </div>
        </div>
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-white/5 bg-[rgba(40,42,44,0.75)]">
                  <th className="px-6 py-4 text-xs uppercase tracking-[0.18em] text-slate-400">Complexity</th>
                  <th className="px-6 py-4 text-xs uppercase tracking-[0.18em] text-slate-400">YALex File Source</th>
                  <th className="px-6 py-4 text-xs uppercase tracking-[0.18em] text-slate-400">Input Stream</th>
                  <th className="px-6 py-4 text-xs uppercase tracking-[0.18em] text-slate-400">Status</th>
                  <th className="px-6 py-4 text-xs uppercase tracking-[0.18em] text-slate-400 text-right">Actions</th>
                </tr>
              </thead>
                    ...
                    <tr key={testCase.id} className="group hover:bg-[rgba(40,42,44,0.72)]">
                      ...
                      <td className="px-6 py-5">
                        <div className="space-y-1">
                          ...
                          <p className="text-[11px] uppercase tracking-[0.16em] text-slate-500">{testCase.expectation}</p>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                      ...
                      <td className="px-6 py-5 text-right">
                        <div className="flex justify-end gap-2">
                          <button
                            type="button"
                            aria-label={`Run case ${testCase.label}`}
                          ...
                          >
                            {runningCaseId === testCase.id ? "Running..." : "Run Case"}
                          </button>
                          <button
                            type="button"
